...
__all__ = ['Conv2dLSQ', 'LinearLSQ', 'ActLSQ', 'LinearMCN', 'LinearLSQ_v2', 'LinearLSQ_v3']
...
class Conv2dLSQ(_Conv2dQ):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, nbits_w=4, mode=Qmodes.kernel_wise, quant_one=False, **kwargs):
...
class LinearLSQ_v3(_LinearQ):
    def __init__(self, in_features, out_features, bias=True, nbits_w=4, **kwargs):
        super(LinearLSQ_v3, self).__init__(in_features=in_features,
    ...
    def forward(self, x):
        ...
        # Method2:
        # w_q = FunLSQ.apply(self.weight, self.alpha, g, Qn, Qp)
        return F.linear(x_q, w_q, self.bias), F.linear(x, self.weight, self.bias)

class LinearLSQ_v2(_LinearQ_v2):
    def __init__(self, in_features, out_features, bias=True, nbits_w=4, quant_one=False, **kwargs):
